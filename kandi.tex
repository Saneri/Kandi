...
Software metric can be any metric that determines software quality in any way. There are defined sets of metrics e.g Halstead metrics \cite{al2005analysis}  but it is also possible to adjust an existing metric for specified software or implement a completely new metric. A software metric can either be dynamic or static \cite{sommerville2011software}. Dynamic metrics are measured when a program is executing and static metrics can be measured e.g by reading the source code or documentation of a program. This thesis focuses on static code analysis.
